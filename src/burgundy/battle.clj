...
(comment
         ...
         (def-task start-stage-task []
           ...
           :action (do (start-stage)
                       (load-skill-types)
                       (dosync
                        (commute battle-state assoc-in [:confined-units] (confined-unit-count))
                        (commute battle-state assoc-in [:attempted-skills] [])
         ...
         (def-task confine-task [id target]
           ...
           :priority 10
           :max-attempts 3
           :goal-state (> (confined-unit-count) units-to-summon)
         ...
         (def-task confine-closest-task [id]
           ...
           :priority 10
           :max-attempts 3
           :goal-state (> (confined-unit-count) (:confined-units @battle-state))
